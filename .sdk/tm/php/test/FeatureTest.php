...
// Freepublicapis SDK feature test
//
...
class FtCtrl extends FreepublicapisControl
{
...
class FtClient
{
    ...
    private FreepublicapisUtility $utility;

    public function __construct(FreepublicapisUtility $utility, array $options)
    {
    ...
    public function get_utility(): FreepublicapisUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FreepublicapisContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FreepublicapisContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FreepublicapisContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FreepublicapisConfig::make_feature($name);
    ...
    private static function buildUrl(FreepublicapisSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FreepublicapisError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FreepublicapisResponse($fetched);
        }
    ...
    private function fail(FreepublicapisContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FreepublicapisContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
