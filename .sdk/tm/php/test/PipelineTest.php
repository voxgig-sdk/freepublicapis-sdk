...
// Freepublicapis SDK pipeline test
//
...
{
    private static function utility(): FreepublicapisUtility
    {
        return new FreepublicapisUtility();
    }
    ...
    private static function ctx(array $over = []): FreepublicapisContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FreepublicapisContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FreepublicapisOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FreepublicapisError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FreepublicapisOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FreepublicapisMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FreepublicapisOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FreepublicapisMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FreepublicapisOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FreepublicapisMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FreepublicapisSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FreepublicapisMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new FreepublicapisResponse([]),
            'result' => new FreepublicapisResult([]),
        ]);
        [, $err] = FreepublicapisMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new FreepublicapisSpec([]),
            'response' => null,
            'result' => new FreepublicapisResult([]),
        ]);
        [, $err] = FreepublicapisMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FreepublicapisSpec(['step' => 's']),
            'response' => new FreepublicapisResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FreepublicapisResult([]),
        ]);
        [, $err] = FreepublicapisMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FreepublicapisSpec(['step' => 's']),
            'response' => new FreepublicapisResponse(self::resp(200, ['v' => 1])),
            'result' => new FreepublicapisResult([]),
        ]);
        [, $err] = FreepublicapisMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FreepublicapisSpec(['step' => 's']),
            'response' => new FreepublicapisResponse(self::resp(200, ['v' => 2])),
            'result' => new FreepublicapisResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FreepublicapisMakeResponse::call($ctx);
    ...
    {
        $preset = new FreepublicapisResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FreepublicapisSpec([]),
            'response' => new FreepublicapisResponse([]),
            'result' => new FreepublicapisResult([]),
        ...
        [$got, $err] = FreepublicapisMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FreepublicapisResult([])]);
        [, $err] = FreepublicapisMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FreepublicapisSpec([]), 'result' => null]);
        [, $err] = FreepublicapisMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FreepublicapisSpec(['step' => 's']),
            'result' => new FreepublicapisResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FreepublicapisOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FreepublicapisMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FreepublicapisSpec(['step' => 's']),
            'result' => new FreepublicapisResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FreepublicapisOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FreepublicapisMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FreepublicapisResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FreepublicapisSpec([]),
            'result' => new FreepublicapisResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FreepublicapisMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FreepublicapisMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FreepublicapisError('boom', 'boom');
        $utility->fetcher = function (FreepublicapisContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FreepublicapisSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FreepublicapisMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FreepublicapisContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FreepublicapisMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FreepublicapisResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        FreepublicapisMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FreepublicapisContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FreepublicapisSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FreepublicapisMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FreepublicapisResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FreepublicapisSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FreepublicapisMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FreepublicapisMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FreepublicapisMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FreepublicapisResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FreepublicapisDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FreepublicapisResult(['ok' => false])]);
        $this->expectException(FreepublicapisError::class);
        FreepublicapisDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FreepublicapisResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FreepublicapisMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        FreepublicapisMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FreepublicapisBaseFeature();
        $b = new FreepublicapisBaseFeature();
        FreepublicapisFeatureAdd::call($ctx, $a);
        FreepublicapisFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FreepublicapisBaseFeature
    {
        $f = new FreepublicapisBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FreepublicapisFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): FreepublicapisContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new FreepublicapisSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FreepublicapisPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        FreepublicapisPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FreepublicapisResponse(['headers' => null]),
            'result' => new FreepublicapisResult([]),
        ]);
        FreepublicapisResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        FreepublicapisResultBody::call($ctx);
