...
// Freepublicapis SDK retry feature
...
class FreepublicapisRetryFeature extends FreepublicapisBaseFeature
{
    ...
    public function init(FreepublicapisContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FreepublicapisContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FreepublicapisContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FreepublicapisContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
