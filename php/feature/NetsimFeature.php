...
// Freepublicapis SDK netsim feature
...
class FreepublicapisNetsimFeature extends FreepublicapisBaseFeature
{
    ...
    private function _simulate(FreepublicapisContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(FreepublicapisContext $ctx, array $applied): void
    {
        $client = $this->client;
