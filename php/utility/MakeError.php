...
class FreepublicapisMakeError
{
    public static function call(?FreepublicapisContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new FreepublicapisContext([], null);
        }
        $op = $ctx->op ?? new FreepublicapisOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new FreepublicapisResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof FreepublicapisError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new FreepublicapisError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof FreepublicapisError) {
