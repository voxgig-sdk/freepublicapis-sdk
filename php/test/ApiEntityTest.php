...
class ApiEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = FreepublicapisSDK::test($seed, null);
        $seen = iterator_to_array($base->Api(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = FreepublicapisConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = FreepublicapisSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FREEPUBLICAPIS_TEST_API_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function api_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("FREEPUBLICAPIS_TEST_API_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["FREEPUBLICAPIS_TEST_API_ENTID"]);
