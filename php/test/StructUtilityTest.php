...
// Freepublicapis SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = FreepublicapisOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(FreepublicapisOmni::structfix($now, Struct::undef()))
                        !== json_encode(FreepublicapisOmni::structfix($val, Struct::undef()))) {
