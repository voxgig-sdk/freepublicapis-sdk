...
[MCP](https://modelcontextprotocol.io) server exposing the Freepublicapis SDK as
two agent tools — `freepublicapis_list` and `freepublicapis_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/freepublicapis-mcp)
make build
...
claude mcp add --scope user freepublicapis \
  -- /absolute/path/to/freepublicapis-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./freepublicapis-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// freepublicapis_list: first page of records
{ "entity": "api" }
{ "entity": "api", "query": { } }

// freepublicapis_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/freepublicapis-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user freepublicapis \
     -- "$PWD"/dist/*/freepublicapis-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `freepublicapis_list` and `freepublicapis_load` tools now appear
   in new sessions. Ask the agent to *"list api using freepublicapis"*
   and it calls `freepublicapis_list` with `{"entity":"api"}`.
...
```sh
export FREEPUBLICAPIS_APIKEY=sk_live_xxx            # API key
export FREEPUBLICAPIS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `freepublicapis_list` tool
...
### Call the `freepublicapis_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `freepublicapis_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `freepublicapis_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FREEPUBLICAPIS_APIKEY` | API key sent with every request. |
| `FREEPUBLICAPIS_BASE` | Optional override of the API base URL. |
...
```sh
./freepublicapis-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"freepublicapis_load","arguments":{"entity":"api","query":{"id":1}}}}'
```
