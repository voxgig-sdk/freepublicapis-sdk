// Command Freepublicapis-cli is a CLI + REPL for the Freepublicapis SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FREEPUBLICAPIS_APIKEY (api key), FREEPUBLICAPIS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FREEPUBLICAPIS_APIKEY carries the API key and
	// FREEPUBLICAPIS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FREEPUBLICAPIS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FREEPUBLICAPIS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFreepublicapisSDK(opts)
