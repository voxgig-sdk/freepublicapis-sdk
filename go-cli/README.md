...
boru-driven command-line client **and** interactive REPL for the Freepublicapis
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/freepublicapis-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/freepublicapis-cli list api
   ```
...
```sh
./freepublicapis-cli load 1 api          # scalar shorthand for {id:1}
./freepublicapis-cli load '{id:1}' api     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `freepublicapis>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/freepublicapis-cli`. |
...
(`./freepublicapis-cli <expr>`) and interactively (the REPL), and expressions compose the
