...
local FreepublicapisSDK = {}
FreepublicapisSDK.__index = FreepublicapisSDK
...
FreepublicapisSDK._make_feature = _make_feature


function FreepublicapisSDK.new(options)
  local self = setmetatable({}, FreepublicapisSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function FreepublicapisSDK:options_map()
...
function FreepublicapisSDK:get_utility()
  return Utility.copy(self._utility)
end


function FreepublicapisSDK:get_root_ctx()
  return self._rootctx
end


function FreepublicapisSDK:prepare(fetchargs)
...
function FreepublicapisSDK:direct(fetchargs)
...
-- by passing it.
function FreepublicapisSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function FreepublicapisSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "FreepublicapisSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function FreepublicapisSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = FreepublicapisSDK.new(sdkopts)
  sdk.mode = "test"
