...
module FreepublicapisUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FreepublicapisResponse.new({})
    ...
    if fetch_err
      ...
      response = FreepublicapisResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FreepublicapisResponse.new(fetched)
    else
