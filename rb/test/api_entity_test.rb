...
class ApiEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = FreepublicapisSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FREEPUBLICAPIS_TEST_API_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def api_basic_setup(extra)
  ...
  entid_env_raw = ENV["FREEPUBLICAPIS_TEST_API_ENTID"]
