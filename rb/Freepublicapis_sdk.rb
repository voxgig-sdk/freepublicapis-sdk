...
require_relative 'Freepublicapis_types'


class FreepublicapisSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FreepublicapisHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FreepublicapisHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FreepublicapisFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FreepublicapisUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FreepublicapisHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FreepublicapisHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FreepublicapisHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => FreepublicapisError.new(
        "#{op}_allow",
        "FreepublicapisSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue FreepublicapisError => err
    ...
    if fetched.is_a?(Hash)
      status = FreepublicapisHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FreepublicapisError.new(
        "graphql_error", "FreepublicapisSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = FreepublicapisSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
