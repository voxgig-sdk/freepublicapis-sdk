# Freepublicapis SDK test runner
...
class FreepublicapisTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FreepublicapisTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FreepublicapisTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FreepublicapisTestRunner.getenv("FREEPUBLICAPIS_TEST_LIVE")
        override = FreepublicapisTestRunner.getenv("FREEPUBLICAPIS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FreepublicapisTestRunner.getenv(key)
        ...
        explain = FreepublicapisTestRunner.getenv("FREEPUBLICAPIS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FREEPUBLICAPIS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FreepublicapisTestRunner._test_control is not None:
            return FreepublicapisTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FreepublicapisTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FreepublicapisTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FreepublicapisTestRunner.load_test_control()
...
def load_env_local():
    FreepublicapisTestRunner.load_env_local()


def env_override(m):
    return FreepublicapisTestRunner.env_override(m)


def entity_data(v):
    return FreepublicapisTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FreepublicapisTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FreepublicapisTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FreepublicapisTestRunner.load_test_control()


def live_client_options():
    return FreepublicapisTestRunner.live_client_options()


def live_delay_ms():
    return FreepublicapisTestRunner.live_delay_ms()
