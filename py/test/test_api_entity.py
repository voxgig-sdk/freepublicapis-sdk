...
class TestApiEntity:
    ...
    def test_should_stream(self):
        ...
        base = FreepublicapisSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FREEPUBLICAPIS_TEST_API_ENTID JSON to run live")
        client = setup["client"]
...
def _api_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("FREEPUBLICAPIS_TEST_API_ENTID"))
    ...
    if env.get("FREEPUBLICAPIS_TEST_LIVE") == "TRUE":
        ...
        client = FreepublicapisSDK(helpers.to_map(merged_opts))

    _live = env.get("FREEPUBLICAPIS_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("FREEPUBLICAPIS_TEST_EXPLAIN") == "TRUE",
        "live": _live,
