...
from freepublicapis_sdk.core.control import FreepublicapisControl
...
from freepublicapis_sdk.core.helpers import get_ctx_prop, to_map
...
class FreepublicapisContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = FreepublicapisControl()
        ...
        if isinstance(sp, FreepublicapisSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, FreepublicapisResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, FreepublicapisResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return FreepublicapisOperation({})
    ...
    def make_error(self, code, msg):
        return FreepublicapisError(code, msg, self)
