...
//     (\`new FreepublicapisSDK(...)\` / \`FreepublicapisSDK.test(...)\` become
//     \`FreepublicapisSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'FreepublicapisSDK'
...
// rewrite real client construction (\`new FreepublicapisSDK(...)\` and any
// \`FreepublicapisSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FreepublicapisSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FreepublicapisSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
