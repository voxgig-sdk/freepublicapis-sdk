...
import (
	...
	omni "github.com/voxgig-sdk/freepublicapis-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FreepublicapisError); is {
				out := map[string]any{
					"name":    "FreepublicapisError",
...
func omniCtx(arg any, client *sdk.FreepublicapisSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FreepublicapisSDK); is {
			client = live
